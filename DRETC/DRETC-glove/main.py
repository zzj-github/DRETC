...
class Trainer(object):
    ...
    def eval(self, data_loader):
        ...
        intra_f1 = (2 * intra_p * intra_r) / (intra_p + intra_r + 1e-20)

        inter_r = inter_correct / inter_recall
        inter_p = inter_correct / inter_mask.sum().item()
        inter_f1 = (2 * inter_p * inter_r) / (inter_p + inter_r + 1e-20)
        ...
        return f1_arr[nt_f1_pos], nt_f1, theta
...
if __name__ == '__main__':
    ...
    for i in range(args.epochs):
        ...
        f1, nt_f1, theta = trainer.eval(dev_loader)
        if f1 > best_F1:
            best_F1 = f1
            best_nt_f1 = nt_f1
            input_theta = theta
            ...
            # dir_ = os.getcwd()
            # dir = os.path.join(dir_, 'save_model/' + str(best_F1) + '.pth')
            # trainer.save(dir)

            trainer.save("model1.pt")
            # trainer.save("model2.pt")


    logger.info("Best NT F1: {:3.4f}".format(best_nt_f1))
    ...
    #                          num_workers=4)
    #
    # # trainer.load("model1.pt")
    # trainer.load("model2.pt")
    #
