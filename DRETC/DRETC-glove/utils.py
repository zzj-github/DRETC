...
def process(data, vocab, is_train):
    ...
    for index, doc in enumerate(data):
        ...
        _ner_inputs = np.zeros((doc_len,), dtype=np.int)    # mark the words belong to entity in the document
        _dis_inputs = np.zeros((men_num, men_num), dtype=np.int)
        _ref_inputs = np.zeros((men_num, men_num), dtype=np.int)    # 1: two mention in same sentence
        ...
        _intrain_mask = np.zeros((ent_num, ent_num, RELATION_NUM), dtype=np.bool)   # labels of no train data
        ...
        _ent2ent_mask = np.ones((ent_num, ent_num), dtype=np.bool)   # NULL
        _men2men_mask = np.ones((men_num, men_num), dtype=np.bool)   # NULL
...
def load_data(load_emb=True):
    ...
    vocab = Vocabulary(rel2id, ner2id, None)
...
def load_embedding():
    embedding = np.load("./data/vec.npy")
